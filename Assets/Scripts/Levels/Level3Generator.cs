...
/// <summary>
/// Generates Level 3 - Obstacle Course
/// Features narrow platforms, gaps, and precision jumping
/// </summary>
public class Level3Generator : MonoBehaviour
{
    [Header("Level Settings")]
    public float blockSize = 1f;
    public int totalFloorBlocks = 140;
    ...
    public void GenerateLevel3()
    {
        ...
        Debug.Log("Level 3 generated successfully!");
    }
    ...
    void CreatePlatforms()
    {
        ...
        // DAY-ONLY BLOCKS: Narrow platforms with gaps
        ...
        // Day platform 2: Small platform after gap
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((10f + i * subBlockSize) * blockSize, 1.5f, 0f), visibleDuringDay: true);
        }
        
        // Day platform 3: Another small platform
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((18f + i * subBlockSize) * blockSize, 2.0f, 0f), visibleDuringDay: true);
        }
        
        // Day platform 4: High platform
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((30f + i * subBlockSize) * blockSize, 2.5f, 0f), visibleDuringDay: true);
        }
        
        // Day platform 5: Gap area
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((50f + i * subBlockSize) * blockSize, 1.8f, 0f), visibleDuringDay: true);
        }
        
        // Day platform 6: Near end
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((80f + i * subBlockSize) * blockSize, 1.2f, 0f), visibleDuringDay: true);
        }
        
        // NIGHT-ONLY BLOCKS: Complete path with narrow platforms
        ...
        // Night platform 1: Narrow platform
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((8f + i * subBlockSize) * blockSize, 1.0f, 0f), visibleDuringDay: false);
        }
        
        // Night jump 1: Small platform
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((12f + i * subBlockSize) * blockSize, 1.5f, 0f), visibleDuringDay: false);
        }
        
        // Night platform 2: Narrow bridge
        for (int i = 0; i < 2 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((16f + i * subBlockSize) * blockSize, 1.5f, 0f), visibleDuringDay: false);
        }
        
        // Night jump 2: Higher platform
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((20f + i * subBlockSize) * blockSize, 2.0f, 0f), visibleDuringDay: false);
        }
        
        // Night staircase 1: Upward climb
        for (int i = 0; i < 5 * densityMultiplier; i++)
        {
            float x = 22f + i * subBlockSize;
            float y = 2.0f + (i * 0.1f);
            CreateBoxBlock(new Vector3(x * blockSize, y, 0f), visibleDuringDay: false);
        }
        
        // Night platform 3: High narrow platform
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((30f + i * subBlockSize) * blockSize, 2.5f, 0f), visibleDuringDay: false);
        }
        
        // Night jump 3: Gap jump
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((35f + i * subBlockSize) * blockSize, 2.3f, 0f), visibleDuringDay: false);
        }
        
        // Night bridge 1: Long narrow bridge
        for (int i = 0; i < 8 * densityMultiplier; i++)
        {
            float x = 37f + i * subBlockSize;
            float y = 2.3f - (i * 0.02f);
            CreateBoxBlock(new Vector3(x * blockSize, y, 0f), visibleDuringDay: false);
        }
        ...
        {
            float x = 45f + i * subBlockSize;
            float y = 2.14f - (i * 0.06f);
            CreateBoxBlock(new Vector3(x * blockSize, y, 0f), visibleDuringDay: false);
        }
        
        // Night platform 4: Lower landing
        for (int i = 0; i < 2 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((51f + i * subBlockSize) * blockSize, 1.78f, 0f), visibleDuringDay: false);
        }
        
        // Night jump 4: Precision jump
        for (int i = 0; i < 1 * densityMultiplier; i++)
        {
            CreateBoxBlock(new Vector3((55f + i * subBlockSize) * blockSize, 1.8f, 0f), visibleDuringDay: false);
        }
        
        // Night bridge 2: Final bridge
        for (int i = 0; i < 20 * densityMultiplier; i++)
        {
            float x = 57f + i * subBlockSize;
            float y = 1.8f - (i * 0.01f);
            CreateBoxBlock(new Vector3(x * blockSize, y, 0f), visibleDuringDay: false);
        }
        ...
        {
            CreateBoxBlock(new Vector3((77f + i * subBlockSize) * blockSize, 1.63f, 0f), visibleDuringDay: false);
        }
    }
    ...
    void PlaceLightningBoltsFallback(List<Vector3> nightBlocks, List<Vector3> dayBlocks)
    {
        ...
        {
            new Vector3(5f, 2.0f, 0f),
            new Vector3(13f, 2.0f, 0f),
            new Vector3(21f, 2.5f, 0f),
            new Vector3(32f, 3.0f, 0f),
            new Vector3(38f, 2.5f, 0f),
            new Vector3(48f, 2.0f, 0f),
            new Vector3(56f, 2.0f, 0f),
            new Vector3(65f, 1.8f, 0f),
            new Vector3(75f, 1.9f, 0f),
            new Vector3(85f, 1.7f, 0f)
        };
    ...
    void PlaceMonsters()
    {
        ...
        CreateMonster(new Vector3(15f, 1.7f, 0f), Monster.MonsterType.FlyingEye);
        CreateMonster(new Vector3(25f, 2.3f, 0f), Monster.MonsterType.FlyingEye);
        CreateMonster(new Vector3(40f, 2.2f, 0f), Monster.MonsterType.FlyingEye);
        CreateMonster(new Vector3(60f, 1.9f, 0f), Monster.MonsterType.FlyingEye);
        CreateMonster(new Vector3(85f, 1.8f, 0f), Monster.MonsterType.FlyingEye);
    ...
    void SetupWinnerScreen()
    {
        ...
        if (existingWinnerScreen == null)
        {
            ...
            winnerScreen.SetCurrentLevel(3);
        }
    }
    ...
    void PlaceFinishLine()
    {
        ...
        finishObj.transform.position = new Vector3(finishX, 2.0f, 0f);
